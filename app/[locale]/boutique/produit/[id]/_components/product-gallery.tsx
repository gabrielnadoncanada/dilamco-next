...
export function ProductGallery({
  ...
  // Les codes produits contiennent des caractères illégaux dans un sélecteur CSS
  // (`.` dans W30X19.5, `*` dans BDD21.75*36) : PhotoSwipe les passe à
  // querySelectorAll, ce qui lève une SyntaxError. On assainit l'id (DOM + sélecteur).
  const galleryID = `pswp-${product.id.replace(/[^a-zA-Z0-9_-]/g, "-")}`;
        ...
        <div className="order-2 flex w-[76px] shrink-0 flex-col gap-3 max-[1000px]:w-full max-[1000px]:flex-row max-[1000px]:flex-wrap">
          {views.map((v, i) => (
            <button
              ...
              aria-label={t("gallery.viewLabel", { label: v.label })}
              aria-current={view === i}
              className={cn(
                  ...
                  {views.map((v, i) => (
                    <CarouselItem key={i} className="pl-0">
                      <GallerySlide
                        ...
                        color={color}
                        molding={molding}
                        zoomLabel={t("gallery.zoomLabel", { label: v.label })}
                      />
                    </CarouselItem>
                  ))}
                </CarouselContent>
